...
class learned_GCN(nn.Module):
    def __init__(self, node_num, in_feature, out_feature):
        ...
        self.in_feature = in_feature  # 输入特征
        self.out_feature = out_feature  # 输出特征

        self.source_embed = nn.Parameter(torch.Tensor(self.node_num, 10))  # 源节点嵌入
        self.target_embed = nn.Parameter(torch.Tensor(10, self.node_num))  # 目标节点嵌入
...
class STCell(nn.Module):
    def __init__(self, node_num=524, seq_len=12, graph_dim=16, tcn_dim=[10], choice=[1, 1, 1], atten_head=2):
        ...
        self.graph_dim = graph_dim   # 节点特征维度
        self.tcn_dim = tcn_dim   # TCN网络维度
        ...
        if choice[1] == 1:
            ...
            self.sp_source_embed = nn.Parameter(torch.Tensor(self.node_num, 12))   # 源节点嵌入
            self.sp_target_embed = nn.Parameter(torch.Tensor(12, self.node_num))   # 目标节点嵌入
...
class STAG_GCN(nn.Module):
    def __init__(self, edge_index, dtw_edge_index, node_num=524, seq_len=12, pred_len=6, graph_dim=32, tcn_dim=[10], atten_head=4,
                 choice=[1, 1, 1]):
        super(STAG_GCN, self).__init__()
        self.edge_index = edge_index
        self.dtw_edge_index = dtw_edge_index
    ...
    def forward(self, x, ):
        ...
        st_output = self.STCell(x, self.edge_index, self.dtw_edge_index)
        output = st_output
