...
if __name__ == "__main__":
    target_pollutant = "PM25"   # PM25 or PM10
    parser = argparse.ArgumentParser()
    parser.add_argument("--output_dir", type=str, default="../dataset/Delhi/train_val_test_data",
                        help="Output directory.")   # 输出目录
    parser.add_argument("--air_quality_df_filename", type=str,
                        default=f"../dataset/Delhi/preprocessed_data/{target_pollutant}_norm.npy",
                        help="Preprocessed air_quality_data readings.", )   # 待处理数据源目录
    parser.add_argument("--num_for_predict", type=int, default=4, help="Sequence Length.", )   # 预测长度
    parser.add_argument("--points_per_hour", type=int, default=4, help="Sequence Length.", )   # 每小时时间步长
    parser.add_argument("--num_of_weeks", type=int, default=5, help="", )
    parser.add_argument("--num_of_days", type=int, default=5, help="", )
    parser.add_argument("--num_of_hours", type=int, default=5, help="", )
    ...
    df = np.load(args.air_quality_df_filename).transpose(1, 0)
    ...
    # The ratios of training, validation and testing set are 2:1:1 for Delhi dataset,
    ...
    for cat in ["train", "val", "test"]:
        ...
        np.savez_compressed(
            os.path.join(args.output_dir, f"{cat}_{target_pollutant}.npz"),
            hour=_hour,
            day=_day,
            week=_week,
            target=_target,
