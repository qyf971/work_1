...
class Dataloader_Recent_Day_Week(object):
    def __init__(self, data_path, flag, batch_size, num_workers, target, predict_len):
    ...
    def read_data(self):
        try:
            ...
            self.hour = torch.tensor(data['hour'], dtype=torch.float32).transpose(1, 3)
            self.day = torch.tensor(data['day'], dtype=torch.float32).transpose(1, 3)
            self.week = torch.tensor(data['week'], dtype=torch.float32).transpose(1, 3)
            self.target_tensor = torch.tensor(data['target'], dtype=torch.float32)[:, :self.predict_len, :, :].transpose(1, 2)   # (B, N, T, F)
...
class Dataloader_Recent(object):
    ...
    def read_data(self):
        try:
            data = np.load(self.data_path)
            self.hour = torch.tensor(data['hour'], dtype=torch.float32).transpose(1, 2)
            self.target_tensor = torch.tensor(data['target'], dtype=torch.float32)[:, :self.predict_len, :, :].transpose(1, 2)
