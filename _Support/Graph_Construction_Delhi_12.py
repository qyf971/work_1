...
def calculate_the_distance_matrix(threshold):
    ...
    print(f"Distance Matrix -  $\sigma$: 8.00, $\epsilon$: {threshold:.2f}")
    distance_matrix_exp = np.exp(-(distance_matrix ** 2) / (8 ** 2))
...
def calculate_the_similarity_matrix(threshold):
    df = pd.read_csv("dataset/Delhi_12/AQI/delhi_aqi.csv")
    data_list = list(df.iloc[:, 1:].values)  # ✅ 已修改
    # 初始化结果矩阵
