...
class ASTGCN_block(nn.Module):
    ...
    def forward(self, x):
        ...
        x_residual = (x_residual + time_conv_output).transpose(1, 2)
...
class ASTGCN_submodule(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.final_conv(x.permute(0, 3, 1, 2))[:, :, :, -1].permute(0, 2, 1).unsqueeze(-1)
...
class ASTGCN(nn.Module):
    def __init__(self, DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, adj_mx, num_for_predict, len_input, num_of_vertices):
        ...
        self.model = make_model(DEVICE, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, adj_mx, num_for_predict, len_input, num_of_vertices)
