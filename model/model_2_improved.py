...
import numpy as np
from model.model_2 import Temporal_Conv_Layer
...
class SpatialBlock(nn.Module):
    ...
    def forward(self, input_spatial_block):
        ...
        if self.spital_transformer_bool and self.GAT_bool:
            ...
            out = self.STransformer(input_spatial_transformer)
        elif self.GAT_bool:
            # GAT 部分
            X_G = torch.Tensor(B, N, 0, d).to(self.device)
            # 对每一个时间步进行图注意力处理
...
class STGAGRTN(nn.Module):
    def __init__(
            ...
            dropout,
            device,
            recent_only, spital_transformer_bool, GAT_bool, temporal_transformer_bool):
        ...
        self.temporal_conv = Temporal_Conv_Layer(in_channels, embed_size, recent_only)
    ...
    def forward(self, x_recent, x_day, x_week):
        ...
        output_spatial_block = self.norm1(self.spatial_block(x) + x)
